...
function resetHearts(){
 hearts.length=0;
 heartData.forEach(h=>hearts.push({...h}));
}
function startGame(){
 gameStarted=true;endingShown=false;hide('title-screen');show('game-hud');hide('ending');
 player.x=960;player.y=820;player.walk=0;player.dir=0;collected=0;resetHearts();
 $('heart-count').textContent='0';closeDialogue(false);
}
function showDialogue(s,m){dialogueOpen=true;$('speaker').textContent=s;$('message').textContent=m;show('dialogue');}
function closeDialogue(checkEnding=true){
 dialogueOpen=false;hide('dialogue');
 if(checkEnding&&collected===5&&!endingShown)revealEnding();
}
function revealEnding(){
 if(endingShown)return;
 endingShown=true;
 setTimeout(()=>show('ending'),300);
}

// Landmark collision keeps the map exploratory without letting the player walk through buildings/water.
function blocked(x,y){
 if(x<42||y<55||x>WORLD_W-42||y>WORLD_H-38)return true;
 if(x>750&&x<1215&&y>225&&y<565)return true; // cabin
 const px=480,py=820,rx=275,ry=190;
 if(((x-px)/rx)**2+((y-py)/ry)**2<1)return true; // pond
 if(Math.abs(x-1510)<62&&Math.abs(y-635)<52)return true; // well
 if(x>335&&x<495&&y>705&&y<825)return true; // cart
 if(Math.abs(x-980)<65&&Math.abs(y-760)<58)return true; // fire
 return false;
}
...
function drawCharacter(x,y,kind,walk){
 ...
 const bob=Math.sin(walk)*2,isBoy=kind==='boy';
 const skin=isBoy?'#bd7a5a':'#c88462',hair=isBoy?'#3a2c31':'#6d4035';
 const shirt=isBoy?'#5e5874':'#c95f4c',pants=isBoy?'#4d566d':'#3f5661';
 ...
 ctx.fillStyle=shirt;ctx.fillRect(-15,-26+bob,30,31);ctx.fillRect(-19,-21+bob,7,16);ctx.fillRect(13,-21-bob,7,16);
 ctx.fillStyle=skin;ctx.fillRect(-20,-7+bob,7,7);ctx.fillRect(14,-7-bob,7,7);
 ctx.fillRect(-5,-29+bob,10,7);ctx.fillRect(-13,-49+bob,26,24);
...
function drawWorld(t){
 ...
 const camX=clamp(player.x-W/2,0,WORLD_W-W),camY=clamp(player.y-H/2,0,WORLD_H-H);
 ...
 particles.forEach(q=>{
   const [x,y]=worldToScreen(q.x,q.y);if(x<-8||x>W+8||y<-8||y>H+8)return;
   ctx.globalAlpha=.18+.18*Math.sin(t*.0013+q.phase);ctx.fillStyle='#fff0ad';
 ...
 if(gameStarted&&!dialogueOpen&&!endingShown&&dist(player,boy)<92){
   ...
   ctx.fillStyle='#ffe8bd';ctx.font='800 10px Nunito,sans-serif';ctx.textAlign='center';ctx.fillText('SPACE  ·  TALK TO HIM',W/2,H-42);
 }
 ctx.textAlign='left';
}
...
function update(dt){
 time+=dt*1000;
 if(!gameStarted||endingShown)return;
 if(dialogueOpen){
   if(keys[' ']||keys.enter){keys[' ']=false;keys.enter=false;closeDialogue(true);}
   return;
 }
 ...
 if(dist(player,boy)<78&&(keys[' ']||keys.enter)){
   ...
   else showDialogue('your boy','You found every little heart. Come here. I made this part just for you. ♡');
   return;
 }
 for(let i=hearts.length-1;i>=0;i--){
   const h=hearts[i];
   if(Math.hypot(player.x-h.x,player.y-h.y)<38){
     const msg=h.text;hearts.splice(i,1);collected++;
